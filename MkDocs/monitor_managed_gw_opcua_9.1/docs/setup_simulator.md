...
打开一个终端窗口（Mac/Linux）或命令窗口（Windows）并输入以下命令：
```` bash
...
您现在应该看到端点的 URL：`opc.tcp://127.0.0.1:50000` - 稍后将在实验中使用。</br>

!!! note 注意
    有关命令不同选项的信息可以在[此处](https://github.com/Azure-Samples/iot-edge-opc-plc#command-line-reference){target=_blank}找到</br>
    与页面中提供的建议命令相比，添加的命令选项包括：
    <ul>
    <li> 务必使用模拟器的 2.5.2 版本 - 如果您使用任何更高版本，您将获得接近 4000 个标签而不是仅 27 个</li>
...
如果您想验证 OPC UA 服务器是否正在运行，有几个选项。</br>
[Prosys OPC UA Browser](https://prosysopc.com/products/opc-ua-browser/){target=_blank} 相当容易使用，并且适用于所有主要操作系统（Windows、Linux、macOS）。

OPC UA Browser 是免费的，但您需要点击请求下载并提供您的联系信息以换取下载。

安装并打开应用程序，然后输入端点的 URL：`opc.tcp://127.0.0.1:50000`</br>
选择 `RandomSignedInt32` 基本遥测，您可以看到一个值属性：</br>
![Prosys OPC UA Browser](img/setup_03.png)</br>
点击 `Refresh Attributes` 并验证每次点击时值都会更改。
